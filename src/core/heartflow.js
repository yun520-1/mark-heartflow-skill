...
var DreamLoopModule, EmotionEngine, SelfHealing, CognitiveEngine, MetacognitiveFailurePredictor;
...
function _initV11432() {
  ...
  try {
    ...
    // v0.13.160: probeAll 改为异步，不阻塞启动
    setImmediate(() => { try { PapersBridge.probeAll(); } catch(_){} });
  } catch(e) {
    PapersBridge = null;
...
class HeartFlow extends EventEmitter {
  ...
  start() {
    ...
    this.metacognitivePredictor = new MetacognitiveFailurePredictor({ rootPath: getRootPath() });
