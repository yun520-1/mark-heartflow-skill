...
class TranslationPipeline {
  ...
  runUserPipeline(input, context = {}) {
    ...
    try {
      ...
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] intentClassifier failed:', e.message);
    }

    try {
      tone = translator.toneAnalyzer?.analyze?.(input, context) || tone;
    } catch (e) {
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] toneAnalyzer failed:', e.message);
    }

    try {
      entities = translator.entityExtractor?.extract?.(input) || entities;
    } catch (e) {
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] entityExtractor failed:', e.message);
    }

    try {
      implicitNeeds = translator.implicitNeedDetector?.detect?.(input, { tone, ...context }) || implicitNeeds;
    } catch (e) {
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] implicitNeedDetector failed:', e.message);
    }

    try {
      confidence = translator.confidenceAnnotator?.annotate?.({ intent, tone, entities, implicitNeeds }, input) || confidence;
    } catch (e) {
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] confidenceAnnotator failed:', e.message);
    }
  ...
  runLLMPipeline(llmOutput, context = {}) {
    ...
    try {
      ...
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] responseCompressor failed:', e.message);
    }

    try {
      confidence = translator.confidenceAnnotator?.annotate?.({ compressed }, llmOutput) || confidence;
    } catch (e) {
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] confidenceAnnotator (LLM) failed:', e.message);
    }

    try {
      userTranslated = translator.llmToUser?.translate?.(compressed, context) || userTranslated;
    } catch (e) {
      // [PROD] 生产环境移除 console.warn: console.warn('[TranslationPipeline] llmToUser failed:', e.message);
    }
