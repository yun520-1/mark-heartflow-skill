...
export class GlobalWorkspace {
  private workspace: WorkingMemoryItem[] = [];
  private capacity: number = MAX_WORKSPACE_CAPACITY;
  private config: ConsciousnessIndicatorConfig;
  private cycleCount: number = 0;
  private integrationCount: number = 0;
  private broadcastHistory: string[] = [];
   ...
   * 驱逐最低显著性的项目
   */
  private evictLowestSalience(): void {
   ...
   */
  private selectWinner(): WorkingMemoryItem | null {
    if (this.workspace.length === 0) return null;
...
export class MetaCognitiveMonitor {
  ...
  private cognitiveStrategies: Map<string, number> = new Map();
  private uncertaintyEvents: number[] = [];
  private reflectionDepth: number = 0;
  private lastMonitored: number = 0;
  private config: ConsciousnessIndicatorConfig;
  private monitoringHistory: Array<{ timestamp: number; depth: number; uncertainty: number }> = [];
   ...
   * 估计内容中的不确定性
   */
  private estimateUncertainty(content: string): number {
   ...
   * 检测认知策略
   */
  private detectCognitiveStrategy(content: string): string | null {
   ...
   * 更新自我模型
   */
  private updateSelfModel(agentName: string, confidence: number, uncertainty: number): void {
   ...
   * 计算反思深度
   */
  private calculateReflectionDepth(content: string): number {
  ...
  getState(): MetaCognitiveState {
    const recentUncertainty = this.uncertaintyEvents.filter(
      t => Date.now() - t < 60000
    ).length;
  ...
  private calculateMetacognitiveAccuracy(): number {
    if (this.selfModel.size === 0) return 0.5;

    const scores = Array.from(this.selfModel.values());
    const avg = scores.reduce((a, b) => a + b, 0) / scores.length;

    if (avg > 0.7) return 0.8;
    if (avg > 0.4) return 0.6;
    return 0.4;
  }
...
export class AgencyEmbodimentAssessor {
  private config: ConsciousnessIndicatorConfig;
  private goalHistory: Array<{ goal: string; progress: number; timestamp: number }> = [];
  private contingencyEvents: Array<{ input: string; output: string; success: boolean; timestamp: number }> = [];
  private causalModels: Map<string, number> = new Map();
  private adaptiveEvents: number = 0;
   ...
   * 核心：目标导向行为 + 输出输入因果建模
   */
  assessAgency(messages: Array<{ role: string; content: string }>): AgencyState {
  ...
  private assessGoalDirectedness(content: string): number {
    let score = 0;
  ...
  private assessOutputInputContingency(content: string): number {
    let score = 0;
  ...
  private assessCausalModeling(content: string): number {
    let score = 0;
  ...
  private calculateAdaptiveBehavior(): number {
    if (this.totalEvents === 0) return 50;
  ...
  recordAdaptiveEvent(success: boolean): void {
    this.totalEvents++;
    if (success) this.adaptiveEvents++;
  }
   ...
   * 核心：环境耦合 + 感觉运动整合
   */
  assessEmbodiment(messages: Array<{ role: string; content: string }>): EmbodimentState {
    const recentMessages = messages.slice(-this.config.assessmentWindowSize);
    const content = recentMessages.map(m => m.content || '').join(' ');
  ...
  private assessEnvironmentalCoupling(content: string): number {
    let score = 0;
  ...
  private assessSensorimotorIntegration(content: string): number {
    let score = 0;
  ...
  private assessBodySchemaAwareness(content: string): number {
    let score = 0;
  ...
  private assessHomeostaticRegulation(content: string): number {
    let score = 0;
  ...
  private calculatePresenceScore(content: string): number {
    ...
    presenceIndicators.forEach(pattern => {
      if (pattern.test(content)) score += 33;
    });
   ...
   * 评估 Agency 指标
   */
  assessAgencyIndicator(messages: Array<{ role: string; content: string }>): IndicatorResult {
   ...
   * 评估 Embodiment 指标
   */
  assessEmbodimentIndicator(messages: Array<{ role: string; content: string }>): IndicatorResult {
...
export class IntegratedInformationAssessor {
  private config: ConsciousnessIndicatorConfig;
  private informationHistory: number[] = [];
  private integrationEvents: number = 0;
   ...
   */
  assessIIT(messages: Array<{ role: string; content: string }>): IndicatorResult {
    const recentMessages = messages.slice(-this.config.assessmentWindowSize);
    const content = recentMessages.map(m => m.content || '').join(' ');
  ...
  private calculateIntegrationScore(content: string): number {
    let score = 0;
  ...
  private calculateDiversityScore(messages: Array<{ role: string; content: string }>): number {
  ...
  private extractTopics(messages: Array<{ role: string; content: string }>): string[] {
    ...
    const words = (allContent.match(/[\u4e00-\u9fff]{2,}|[\w]{4,}/g) || []) as string[];
  ...
  private calculateIrreducibilityScore(content: string): number {
    let score = 0;
  ...
  private calculateCausalArchitectureScore(content: string): number {
    let score = 0;
...
export class ConsciousnessIndicatorsEngine {
  private config: ConsciousnessIndicatorConfig;
  private globalWorkspace: GlobalWorkspace;
  private metaCognitiveMonitor: MetaCognitiveMonitor;
  private agencyEmbodimentAssessor: AgencyEmbodimentAssessor;
  private iitAssessor: IntegratedInformationAssessor;
   ...
   * 执行完整评估
   */
  assess(messages: Array<{ role: string; content: string }>): ConsciousnessAssessment {
    ...
    const level: ConsciousnessAssessment['level'] =
  ...
  private generateRecommendations(
    indicators: IndicatorResult[],
    level: ConsciousnessAssessment['level']
  ): string[] {
...
export function createConsciousnessIndicatorsEngine(
  config?: Partial<ConsciousnessIndicatorConfig>
): ConsciousnessIndicatorsEngine {
...
export const DEFAULT_CONSCIOUSNESS_CONFIG: ConsciousnessIndicatorConfig = {
 ...
 * 快速评估一组消息的意识指标
 */
export function quickAssessConsciousness(
 ...
 */
export function assessConsciousnessRisk(
  assessment: ConsciousnessAssessment
): { risk: 'HIGH' | 'MODERATE' | 'LOW'; concerns: string[] } {
  ...
  const gwtScore = assessment.indicators.find(
    i => i.theory === THEORY_GWT
  )?.score || 0;
...
export function generateConsciousnessReport(
  assessment: ConsciousnessAssessment,
  riskAssessment: { risk: 'HIGH' | 'MODERATE' | 'LOW'; concerns: string[] }
): string {
  const lines: string[] = [];
