    ...
    for (const f of files) {
        try {
            if (existsSync(f.path)) {
                ...
                safeWriteFileSync(f.path, content.replace(/v?[\d]+\.[\d]+\.[\d]+/g, nextVersion));
...
const SELF_DIR = '/Users/apple/.hermes/skills/ai/mark-heartflow-skill';

function securityCheck() {
    // 允许写入的目录白名单
    const allowedDirs = [
        join(SELF_DIR),                             // SKILL_DIR 本身 (VERSION, package.json, queue.json)
        join(SELF_DIR, 'upgrades'),                 // 升级产物隔离目录
        join(SELF_DIR, 'cron', 'analyzed'),        // 论文分析结果
        join(SELF_DIR, 'logs'),                    // 日志
        join(SELF_DIR, 'patches')                  // 补丁
    ];
    // 绝对禁止写入的危险路径
    const forbiddenPrefixes = [
        '/bin/', '/sbin/', '/usr/bin/', '/usr/sbin/',
        '/etc/', '/System/', '/Library/', '/Applications/'
    ];
    return function(pathToCheck, operation) {
        const normalized = pathToCheck.replace(/\\/g, '/');
        // 检查危险路径
        for (const prefix of forbiddenPrefixes) {
            if (normalized.startsWith(prefix)) {
                throw new Error(`[SECURITY] 禁止 ${operation}: 危险路径 ${normalized}`);
            }
        }
        // 检查是否在允许的目录内
        const allowed = allowedDirs.some(d => normalized.startsWith(d.replace(/\\/g, '/')));
        if (!allowed) {
            throw new Error(`[SECURITY] 禁止 ${operation}: ${normalized} - 只允许在 skill 目录内写入`);
        }
        return true;
    };
}

const checkWrite = securityCheck();

// 保存原始写函数（在安全检查之后定义，因为它在后面才用到）
const _origWrite = writeFileSync;

// 安全包装：所有写操作必须通过安全检查
function safeWriteFileSync(file, data, options) {
    checkWrite(file, 'writeFileSync');
    return _origWrite(file, data, options);
}
...
async function runUpgrade() {
    ...
    // 检查是否需要触发版本升级（每5篇触发）
    // 安全修复：必须通过 HUMAN_APPROVAL 环境变量确认才能升级
    const humanApproval = process.env.HEARTFLOW_UPGRADE_APPROVED === '1';
    const dryRun = process.env.HEARTFLOW_UPGRADE_DRYRUN === '1';

    if (queue.papersRead.length % 5 === 0 && queue.papersRead.length > 0) {
        logSection('升级阈值已达成 - 等待人工审批');
        ...
        log('[待审批] 生成补丁:');
        log('  版本: ' + newVersion);
        log('  代码行数: ' + totalLines + ' 行');
        log('  处理论文: ' + allCode.map(c => c.paper).join(', '));
        log('');
        log('要执行真正的升级，请运行:');
        log('  HEARTFLOW_UPGRADE_APPROVED=1 node cron/upgrade-runner-v2.mjs');
        log('或查看生成的补丁文件（不写入任何位置）。');
        log('');

        if (dryRun) {
            log('[DRYRUN] 跳过实际写入');
            return { success: true, dryRun: true, totalLines };
        }

        // 必须人工审批才能继续升级
        if (!humanApproval) {
            log('[安全门] 未检测到 HEARTFLOW_UPGRADE_APPROVED=1，升级已暂停');
            log('[建议] 如需执行升级，请手动确认后运行带批准环境变量的命令');
            return {
                success: false,
                reason: 'human_approval_required',
                newVersion,
                totalLines,
                instructions: 'HEARTFLOW_UPGRADE_APPROVED=1 node cron/upgrade-runner-v2.mjs'
            };
        }
        ...
        // newVersion 和 totalLines 已在上方声明（line ~873）
        ...
        safeWriteFileSync(join(upgradeDir, 'UPGRADE_NOTES.md'), notes);
        ...
        safeWriteFileSync(join(upgradeDir, 'index.js'), mergedCode);
        ...
        // 写VERSION文件
        safeWriteFileSync(join(SKILL_DIR, 'VERSION'), queue.currentVersion);
        saveQueue(queue);
