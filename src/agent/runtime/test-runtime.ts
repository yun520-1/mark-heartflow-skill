 ...
 * Runtime 集成测试
 */
import { TaskGraph, TaskScheduler, AgentRuntime } from './index';
...
const graph2 = new TaskGraph({ name: 'test-scheduler' });
...
const runtime = new AgentRuntime({ name: 'TestRuntime' });

const workflow = AgentRuntime.createDAGWorkflow('wf1', 'My Workflow', [
...
runtime.on('heartbeat', (e) => {
  console.log(`[HEARTBEAT] uptime: ${(e.payload as any).uptime}ms`);
});
