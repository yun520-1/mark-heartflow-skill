...
> **记忆永久化**：安装心虫后，每次 `hf.think()` 自动记录对话关键信息到 `memory/` 目录下的 JSON 文件。
> 下次启动时记忆自动加载，用 `heartflow-memory-tool.js` 随时查询。纯文本存储，约 290 bytes/条，一年约 2MB。
> 记忆目录下的 `existence-log.jsonl` 自动保留最近100条，避免噪音膨胀。
