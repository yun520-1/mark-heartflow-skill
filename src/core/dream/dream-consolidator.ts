...
function buildTagFrequency(blocks: EpisodicBlock[]): Map<string, { freq: number; blockIds: string[] }> {
...
function extractPatterns(
  tagFreq: Map<string, { freq: number; blockIds: string[] }>,
  threshold: number,
): Pattern[] {
...
function findConnections(blocks: EpisodicBlock[], patterns: Pattern[]): Connection[] {
...
function findMergeCandidates(
  ...
  const candidates: [EpisodicBlock, EpisodicBlock][] = [];
...
function mergeBlocks(
  ...
  const mergedEntries = [...a.entries, ...b.entries].sort(
    (x, y) => x.timestamp - y.timestamp,
  );

  const merged: EpisodicBlock = {
    id: generateId(),
