...
const FALLACY_PATTERNS = {
  zh: [
    ...
    [/[身边周围]的[^，。]*?都[^，。]*?说明[^，。]*?(都|全)/i, 'hasty_generalization'],
    [/[一二两三个]个[^，。]*?(例子|案例)[^，。]*?(就能|足以|说明)/i, 'hasty_generalization'],
    // 虚假二分补充 — 只有两个选项没有中间
    [/不是[^，。]*?就是[^，。]*?(没有中间道路|非此即彼)/i, 'false_binary'],
    [/只有[^，。]*?或[^，。]*?没有其他(选择|可能|选项)/i, 'false_binary'],
    // 诉诸怜悯 — 因为可怜所以正确
    [/你忍心[^，。]*?(吗|么)|难道你就不为[^，。]*?想想/i, 'appeal_to_pity'],
    [/[可怜辛苦好不容易](?!.*但是)[^，。]*?(所以|因此|难道不应该)/i, 'appeal_to_pity'],
    [/我这么[^，。]*?(辛苦|努力|不容易)[^，。]*?你还好意思/i, 'appeal_to_pity'],
    // 基因谬误 — 因为来源有问题所以无效
    [/[这那]个[^，。]*?(说法|观点|理论|数据)[^，。]*?来自[^，。]*?(;所以|;因此|不可信|没用)/i, 'genetic_fallacy'],
    [/[^，。]*?不就是[^，。]*?出身(的)?[^，。]*?能有什么(价值|水平)/i, 'genetic_fallacy'],
    [/[^，。]*?是[^，。]*?说的[^，。]*?(那就|肯定)(不对|没用|不可信)/i, 'genetic_fallacy'],
    // 滑坡谬误扩展
    [/[^，。]*?如果[^，。]*?(让步|妥协|退让)[^，。]*?(最终|迟早|早晚)[^，。]*?(无可挽回|不可收拾|毁灭)/i, 'slippery_slope'],
    [/今天[^，。]*?明天[^，。]*?后天[^，。]*?(就|就会)(不可收拾|无法控制|全完了)/i, 'slippery_slope'],
    // 完美主义谬误扩展
    [/不能[^，。]*?(完全|彻底|100%)[^，。]*?还不如[^，。]*?(不做|不要)/i, 'perfectionist_fallacy'],
    // 诉诸动机 — 质疑动机来否定论点
    [/你不就是[^，。]*?为了[^，。]*?(才|所以)[^，。]*?说的/i, 'appeal_to_motive'],
    [/[^，。]*?说这话[^，。]*?(不就是|还不是|无非是)[^，。]*?为了[^，。]*?利益/i, 'appeal_to_motive'],
    [/你这么[^，。]*?不就是[^，。]*?想[^，。]*?吗/i, 'appeal_to_motive'],
    // 叙事谬误 — 把好听的故事当证据
    [/[有个听过]个[^，。]*?故事[^，。]*?(说明|证明|告诉我们)[^，。]*?道理/i, 'narrative_fallacy'],
    ...
    // 沉没成本谬误 — 已经投入这么多所以不能停
    [/已经花了[^，。]*?(这么多|这么久|这么多钱)[^，。]*?(不能|怎能|怎么)(放弃|停止|回头)/i, 'sunk_cost_fallacy'],
    [/投入了[^，。]*?(这么多|这么久)[^，。]*?(现在放弃|半途而废)[^，。]*?太可惜了/i, 'sunk_cost_fallacy'],
  ],
  en: [
