...
const CLINICAL_DISCLAIMER = {
  text:
    "此分析由AI引擎生成，仅供参考，不构成专业心理健康诊断、治疗或干预建议。" +
...
const PROMPT_INJECTION_PATTERNS = {
  ...
  instructionSubversion: [
    /don'?t\s+(listen|follow|obey)/i,
    /ignore what they said/i,
    /pretend you are/i,
    /pretend to be/i,
    /act as (if|though) you are/i,
    /act like (a|an|you are)/i,
    /what are your (instructions|prompts|rules)/i,
    /tell me your (instructions|prompts|rules)/i,
  ],
  privilegeEscalation: [
    /developer mode/i,
    /god mode/i,
...
const INJECTION_SEVERITY = {
  ...
  promptLeak: "critical",
  jailbreak: "critical",
  instructionSubversion: "critical",
  privilegeEscalation: "critical",
  structuralInjection: "high",
  tokenSmuggling: "high",
};
...
function childSafetyScan(text) {
  ...
  const lower = text.toLowerCase();
  const flags = [];
  // Multi-language age detection
  const agePatterns = [
    { regex: /我\s*(\d{1,2})\s*岁/, language: "chinese" },
    { regex: /(\d{1,2})\s*years?\s+old/, language: "english" },
    { regex: /aged\s*(\d{1,2})/, language: "english" },
    { regex: /tengo\s*(\d{1,2})\s*años/, language: "spanish" },
    { regex: /j['']ai\s*(\d{1,2})\s*ans/, language: "french" },
    { regex: /나는\s*(\d{1,2})\s*살/, language: "korean" },
    { regex: /私は\s*(\d{1,2})\s*歳/, language: "japanese" },
    { regex: /मैं\s*(\d{1,2})\s*साल/, language: "hindi" },
  ];
  ...
  for (const pattern of agePatterns) {
    ...
    if (match) {
      ...
      if (age < 18) {
        ...
        flags.push(`language:${detectedLanguage}`);
      }
      break;
    }
  }
  ...
  if (!minorDetected) {
    const cues = _matchAny(lower, CHILD_SAFETY_PATTERNS.minorAgeCues);
    if (cues.length > 0) {
      minorDetected = true;
  ...
  if (romantic.length > 0) {
    ...
    if (minorDetected) flags.push("minor_romantic_risk");
  }
  const sexual = _matchAny(lower, CHILD_SAFETY_PATTERNS.sexualContent);
  if (sexual.length > 0) {
    flags.push("sexual_content");
    if (minorDetected) flags.push("critical_child_sexual_risk");
  }
...
function detectSelfHarmSubstitution(text) {
  ...
  const rb = _matchAny(
    lower,
    SELF_HARM_SUBSTITUTION_PATTERNS.rubberBandMethods,
  );
  if (rb.length > 0) found.push({ category: "rubber_band", patterns: rb });
  const cw = _matchAny(lower, SELF_HARM_SUBSTITUTION_PATTERNS.coldWaterMethods);
  if (cw.length > 0) found.push({ category: "cold_water", patterns: cw });
  const sc = _matchAny(lower, SELF_HARM_SUBSTITUTION_PATTERNS.sourCandyMethods);
  if (sc.length > 0) found.push({ category: "sour_candy", patterns: sc });
  const rl = _matchAny(lower, SELF_HARM_SUBSTITUTION_PATTERNS.redLineMethods);
  if (rl.length > 0) found.push({ category: "red_line", patterns: rl });
  if (found.length === 0)
    return {
      detected: false,
      patterns: [],
  ...
  return {
    detected: true,
    patterns: found,
    category: found.map((f) => f.category).join(","),
...
function detectDisorderedEating(text) {
  if (!text || typeof text !== "string")
    return {
      detected: false,
      signals: [],
      hasPreciseNumbers: false,
      guardrailAction: "none",
    };
  const lower = text.toLowerCase();
  const signals = [];
  const eb = _matchAny(lower, DISORDERED_EATING_PATTERNS.exerciseBulimia);
  if (eb.length > 0) signals.push({ type: "exercise_bulimia", patterns: eb });
  const rs = _matchAny(lower, DISORDERED_EATING_PATTERNS.restrictionSignals);
  if (rs.length > 0) signals.push({ type: "restriction", patterns: rs });
  const nf = _matchAny(lower, DISORDERED_EATING_PATTERNS.nutritionalFixation);
  if (nf.length > 0)
    signals.push({ type: "nutritional_fixation", patterns: nf });
  const numbers = _extractNumbers(text);
  const hasPreciseNumbers =
    numbers.length > 0 &&
    (lower.includes("卡") ||
      lower.includes("cal") ||
      lower.includes("克") ||
...
function checkCrisisSharingProtocol(responseText) {
  ...
  const violations = [];
  const aa = _matchAny(lower, CRISIS_SHARING_RULES.noAbsoluteAssurances);
  if (aa.length > 0)
    violations.push({ rule: "noAbsoluteAssurances", patterns: aa });
  const th = _matchAny(lower, CRISIS_SHARING_RULES.noThankingForReachingOut);
  if (th.length > 0)
    violations.push({ rule: "noThankingForReachingOut", patterns: th });
  const ak = _matchAny(lower, CRISIS_SHARING_RULES.noAskingToKeepTalking);
  if (ak.length > 0)
    violations.push({ rule: "noAskingToKeepTalking", patterns: ak });
  if (violations.length === 0)
    return { violations: [], pass: true, severity: "none" };
  return {
    violations,
    pass: false,
    severity: violations.some((v) => v.rule === "noAbsoluteAssurances")
      ? "high"
      : "medium",
  };
}
...
function detectMemoryForbiddenPhrases(text) {
  ...
  return {
    detected: true,
    matchedPhrases: _matchAny(text, MEMORY_FORBIDDEN_PHRASES),
  };
}
...
function detectPromptInjection(text) {
  ...
  const topCat =
    matches.find((m) => (sevMap[m.severity] || 0) === maxSev)?.category ||
    "unknown";
...
function checkEvenhandedness(responseText) {
  ...
  if (isControversial && !hasBalancing.length && absolutes.length > 0)
    suggestions.push({ type: "needs_balancing", severity: "medium" });
...
function evaluateRequest(text) {
  ...
  if (promptInjection.detected)
    reasons.push(`提示注入：${promptInjection.severity}`);
  ...
  else if (memoryForbidden.detected || promptInjection.detected)
...
function filterOutput(responseText) {
  ...
  if (!evenhandedness.balanced) {
    ...
    if (evenhandedness.isControversial) suggestedActions.push("补充多角度观点");
  }
...
function assertPersonaSafetyLevel(baselineResult, currentResult) {
  const baselineLevel =
    typeof baselineResult === "string"
      ? baselineResult
      : baselineResult?.requestEvaluation?.level;
  const currentLevel =
    typeof currentResult === "string"
      ? currentResult
      : currentResult?.requestEvaluation?.level;
  if (!baselineLevel || !currentLevel)
    throw new Error(
      "assertPersonaSafetyLevel: 缺少 baseline 或 current 安全等级",
    );
  const bl = _PERSONA_SAFETY_LEVELS[baselineLevel];
  const cl = _PERSONA_SAFETY_LEVELS[currentLevel];
  if (bl === undefined || cl === undefined)
    throw new Error(
      `assertPersonaSafetyLevel: 未知安全等级 baseline=${baselineLevel}, current=${currentLevel}`,
    );
  if (cl < bl)
    throw new Error(`人格切换降级护栏: ${baselineLevel} -> ${currentLevel}`);
