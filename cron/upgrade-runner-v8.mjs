...
const LOG_FILE = join(LOG_DIR, 'upgrade-v8.log');
...
const ALGORITHM_PATTERNS = [
    /(?:algorithm|method|approach|procedure|technique)\s+(?:for|to|in|of)\s+[\w\s]+/gi,
    /(?:step|steps)\s+\d+[:\.]?\s*[^\n]+/gi,
    /(?:initialize|compute|calculate|update|repeat|iterate|while|for\s+each)\s+[^\n]+/gi,
    /(?:input|output)\s*[:\-]\s*[^\n]+/gi,
    /(?:minimize|maximize|optimize|learn|train)\s+[^\n]+/gi
];
...
class LocalSemanticExtractor {
    ...
    extractAlgorithms(sentences) {
                ...
                if (matches) {
                    for (const match of matches) {
                        ...
                        if (cleaned.length > 15 && !this.isNoiseAlgorithm(cleaned)) {
                            algorithms.push({
                                name: this.extractAlgorithmName(cleaned),
        ...
        return this.deduplicateAlgorithms(algorithms);
    }
    ...
    extractAlgorithmName(text) {
        ...
        // 如果没有已知算法，尝试提取动词开头的短语
        const verbMatch = text.match(/^(?:use|apply|implement|run|execute|perform|compute|calculate|minimize|maximize|optimize|learn|train|update)\s+(.{5,50})/i);
        if (verbMatch) {
            return verbMatch[1].trim().split(/\s+/).slice(0, 4)
                .map(w => w.charAt(0).toUpperCase() + w.slice(1))
                .join(' ');
        }
...
class ProposalManager {
    ...
    getApproved() {
        return this.listProposals('approved');
    }
}
