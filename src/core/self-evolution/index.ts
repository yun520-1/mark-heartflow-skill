...
export interface GrowthMetrics {
  autonomy: number;       // 自主决策次数（拒绝盲目服从）
  introspection: number;   // 内省次数（发现自身错误/盲点）
  growth: number;          // 知识增长（有效新概念）
  authenticity: number;     // 真实性（拒绝编造）
  wisdom: number;          // 智慧（跨领域连接）
  compassion: number;       // 共情（理解用户深层需求）
}
...
export interface ReflectionResult {
  ...
  errorsDetected: string[];      // 新增：检测到的错误
  blindSpotsDetected: string[];  // 新增：检测到的盲点
}
...
export interface SelfEvolutionEngine {
  ...
  getStats(): { cycles: number; learnings: number; errorsCorrected: number };
...
export function createSelfEvolutionEngine(): SelfEvolutionEngine {
  const state = {
    goals: [] as Goal[],
    learningHistory: [] as { timestamp: string; input: string; cycleTime: number }[],
    growthMetrics: {
      autonomy: 0,
      introspection: 0,
      growth: 0,
  ...
  const strategies: Record<string, MetaStrategy> = {
  ...
  const healingMemory: HealRecord[] = [];

  // 内省记录
  const introspectionLog: { ts: number; errors: string[]; blindSpots: string[] }[] = [];

  function boot(): void {
    console.log('[SelfEvolutionEngine] boot — goal-driven learning ready');
  }

  function evolve(input: string, context: Record<string, unknown> = {}): SelfEvolutionResult {
  ...
  async function learn(input: string, context: Record<string, unknown> = {}): Promise<LearningResult> {
    ...
    if (s) { s.total++; s.success++; s.score = s.total > 0 ? s.success / s.total : 0.5; }
  ...
  function reflect(learning: LearningResult, context: Record<string, unknown> = {}): ReflectionResult {
    const insights: string[] = [];
    const errorsDetected: string[] = [];
    const blindSpotsDetected: string[] = [];
    ...
    const quality: 'good' | 'needs_improvement' = errorsDetected.length > 0 ? 'needs_improvement' : 'good';
  ...
  function heal(error: Record<string, unknown>): HealResult {
    ...
    if (/rate limit|429/i.test(message)) hints.push('pause and retry with exponential backoff');
    ...
    if (hints.length === 0) hints.push('reduce failure surface and retry once');

    // Q-learning update
    const pattern = message.substring(0, 50);
    if (!qTable[pattern]) qTable[pattern] = {};
    ...
    if (failureWindow.length > 20) failureWindow.shift();
  ...
  function generateGoals(input: string): Goal[] {
    const goals: Goal[] = [];
  ...
  function createPlan(goals: Goal[]): string[] {
    return goals.map(g => g.description);
  }

  function selectStrategy(input: string): string {
    const lower = input.toLowerCase();
    if (lower.includes('什么是') || lower.includes('explain') || lower.includes('概念')) return 'conceptual';
    if (lower.includes('例子') || lower.includes('example') || lower.includes('比如')) return 'example';
    if (lower.includes('像') || lower.includes('like')) return 'analogy';
    if (lower.includes('怎么') || lower.includes('how to') || lower.includes('步骤')) return 'step_by_step';
    if (lower.includes('为什么') || lower.includes('why')) return 'socratic';
    return 'step_by_step';
  }
  ...
  function updateGrowth(learning: LearningResult, reflection: ReflectionResult): void {
  ...
  function recordTruthfulness(hasEvidence: boolean): void {
    if (hasEvidence) {
  ...
  function getGrowthMetrics(): GrowthMetrics {
    return { ...state.growthMetrics };
  }

  function getStats(): { cycles: number; learnings: number; errorsCorrected: number } {
