...
const BigFivePersonality = {
  ...
  /**
   * 获取协作建议
   * @param {object} profile - 人格档案
   * @returns {object} 协作建议
   */
  getCollaborationTips(profile) {
    const tips = {};
    if (profile.O?.score >= 6) tips.O = '提供新颖选项，鼓励创新';
    else tips.O = '提供具体步骤，减少不确定性';

    if (profile.C?.score >= 6) tips.C = '给出清晰步骤，跟进执行';
    else tips.C = '允许灵活调整，减少结构化约束';

    if (profile.E?.score >= 6) tips.E = '积极互动，及时反馈';
    else tips.E = '给予独立思考空间，不过度打扰';

    if (profile.A?.score >= 6) tips.A = '强调双赢，避免竞争';
    else tips.A = '尊重边界，不过度协商';

    if (profile.N?.score >= 6) tips.N = '提供安全感，温和确认';
    else tips.N = '直接沟通，减少模糊';
