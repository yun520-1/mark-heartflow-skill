...
exports.createDreamEngineFactory = createDreamEngine;
...
function lightPhase(blocks, state) {
    state.stages.Light++;
...
// REM阶段：跨域连接，发现隐藏关系
function remPhase(blocks, state) {
    state.stages.REM++;
    const pairs = [];
...
// Deep阶段：矛盾检测，发现前提假设
function deepPhase(pairs, state) {
    state.stages.Deep++;
    const insights = [];
    const now = Date.now();
...
// Lucid阶段：自我反思，发现自身模式
function lucidPhase(blocks, insights, state) {
    state.stages.Lucid++;
    const lucidInsights = [];
    const now = Date.now();
    // 发现高频标签 → 执念模式
...
// Wide阶段：超越逻辑，生成反事实推理
function widePhase(blocks, insights, state) {
    state.stages.Wide++;
    const wideInsights = [];
    // 反事实：如果去掉最重要的约束，会怎样？
 ...
 */
// 计算两个记忆块的连接分数
function calculateConnectionScore(a, b) {
    let score = 0;
    // 标签重叠
    const tagsA = new Set(a.tags ?? []);
    const tagsB = new Set(b.tags ?? []);
...
// 判断两个内容是否矛盾（简单检测）
function areContradictory(contentA, contentB) {
...
function createDreamEngine(config) {
    ...
    const fragments = [];
    async function runNightDream(recallBlocks = new Map()) {
    ...
    function addFragment(block) {
        fragments.push(block);
    }
    const engine = Object.assign(new events_1.EventEmitter(), {
        state,
        runNightDream,
