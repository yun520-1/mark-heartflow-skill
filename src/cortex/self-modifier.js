...
`
  ...
  async metacognitiveModify(suggestion) {
    ...
    const patch = this.createMetaPatch(originalContent, modifiedContent, parsed);
  ...
  createMetaPatch(original, modified, parsed) {
...
#
# 撤销补丁:
#   patch -R -p1 < patches/${patchFileName}
#
# ============================================================================
--- a/${parsed.file}
+++ b/${parsed.file}
...
`;
  ...
  log(message) {
    try {
      const dir = path.dirname(this.changeLog);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
      }
      const timestamp = new Date().toISOString();
      fs.appendFileSync(this.changeLog, `[${timestamp}] ${message}\n`);
    } catch (e) {
  ...
  recordChange(change) {
    try {
      ...
      const recordPath = path.join(path.dirname(this.changeLog), 'changes.jsonl');
      const timestamp = new Date().toISOString();
      const record = { ...change, recordedAt: timestamp };
  ...
  describeChanges(original, modified) {
    const origLines = original.split('\n').length;
    const modLines = modified.split('\n').length;
    const linesChanged = Math.abs(modLines - origLines);
    return `${linesChanged} 行发生变化 (${origLines} → ${modLines})`;
  }
}
