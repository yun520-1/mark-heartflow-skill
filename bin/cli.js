...
function summarizeState(emotion, flow) {
  if (flow.state === 'anxiety') {
    return '你当前更像是“负荷偏高、需要减压和拆解”的状态。';
  }
  if (flow.state === 'boredom') {
    return '你当前更像是“挑战不足、需要重新聚焦价值”的状态。';
  }
  if ((emotion.dominant || 'neutral') === 'excited') {
    return '你当前能量偏高，适合先收束重点，再进入执行。';
  }
  if ((emotion.dominant || 'neutral') === 'calm') {
    return '你当前相对平稳，适合做结构化判断与规划。';
  }
  if (emotion.pleasure < 0) {
    return '你当前带着一点阻力感，先减轻内耗，再推进更有效。';
  }
...
function buildFocus(text, flow) {
  if (flow.state === 'anxiety') {
    return '先缩小问题边界，只处理最关键的一个切口。';
  }
  if (flow.state === 'boredom') {
    return '重新定义目标，让任务与真实价值重新对齐。';
  }
  if (/bug|报错|错误|故障|异常/i.test(text)) {
    return '先复现问题，再定位根因，不要同时改多处。';
  }
  if (/方向|选择|决策|不知道先做什么/i.test(text)) {
    return '先定唯一优先级，再把其它方向暂时降级。';
  }
  if (/学习|理解|研究/i.test(text)) {
    return '先明确你要得到的结论，再决定需要读什么。';
  }
...
function buildWarning(flow) {
  if (flow.state === 'anxiety') {
    return '风险：如果继续同时推进太多事，容易放大混乱和错误。';
  }
  if (flow.state === 'boredom') {
    return '风险：如果目标过轻或过散，执行会失去真实驱动力。';
  }
  if (flow.recommendations?.length) {
    return `风险：${flow.recommendations[0]}`;
  }
