...
// 2. 关键文件存在
['src/core/heartflow.js', 'bin/cli.js', 'package.json'].forEach(f => {
  check(`${f} 存在`, () => {
    if (!fs.existsSync(path.join(HF_DIR, f))) throw new Error('文件不存在');
  });
});
...
check('模块数 >= 40', () => {
  ...
  if (count < 40) throw new Error(`只有 ${count} 个模块`);
});
